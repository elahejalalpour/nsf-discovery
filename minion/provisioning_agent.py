...
class ProvisioningAgent():
    ...
    def provision_single_vnf(self, vnf_config, chain_rollback):
        ...
        if "cpu_share" in vnf_config_keys:
            host_config['CpuShares'] = int(
                1024 * (float(vnf_config['cpu_share']) / 100.0))
        ...
        # /var/run/netns
        cont_pid = str(
            self.__container_driver.get_container_pid(container_name))
        ...
        for net_config in vnf_config['net_ifs']:
            veth_vs, veth_cn = self.generate_unique_veth_endpoints(
                container_name, self.__default_ovs_bridge)
            # print "Creating veth pair (" + veth_cn + ", " + veth_vs + ")\n"
            self.__veth_driver.create_veth_pair(veth_cn, veth_vs)
            chain_rollback.push(
                self.__veth_driver.delete_veth_interface,
                veth_cn)
            chain_rollback.push(
                self.__veth_driver.delete_veth_interface,
                veth_vs)

            self.__veth_driver.move_veth_interface_to_netns(
                veth_cn,
                netns=cont_pid)
            self.__veth_driver.update_mtu_at_veth_interface(
                veth_cn,
                1436,
                netns=cont_pid)
            self.__veth_driver.enable_veth_interface(veth_cn, netns=cont_pid)

            self.__veth_driver.assign_ip_to_veth_interface(
                veth_cn,
                net_config['ip_address'],
                netns=cont_pid)
            chain_rollback.push(
                self.__veth_driver.delete_ip_from_veth_interface,
                net_config['ip_address'],
                netns=cont_pid)
        ...
        with RollbackContext() as chain_rollback:
            ...
            for i in range(0, len(updated_chain_config)):
                ...
                for j in range(0, len(vnf_conf['net_ifs'])):
                    ...
                    if link_id not in links.keys():
                        ...
                        links[link_id]["endpoint_a"] = vnf_conf[
                            "container_name"]
                        ...
                        links[link_id]["endpoint_b"] = vnf_conf[
                            "container_name"]
            ...
            for (link_id, link) in links.iteritems():
                if link["link_type"] == "local":
                    ...
                    self.__chain_driver.connect_containers_inside_host(
                        a,
                        veth_vs_a,
                        b,
                        veth_vs_b,
                    ...
                    remote_container_ip = link[
                        "remote_container_ip"].split("/")[0]
                    bandwidth = link["bandwidth"]
                    tunnel_id = str(link_id)
                    tunnel_interface_name = self.__default_tunnel_interface
                    self.__chain_driver.connect_containers_across_hosts(
                        a,
                        veth_vs,
                        ovs_bridge_name,
                        container_ip_net,
    ...
    def generate_unique_veth_endpoints(self, container_name, ovs_bridge_name):
        ...
        while(True):
            ...
            present_in_host = self.__veth_driver.is_interface_present(
                veth_endpoint_a)
            present_in_ovs = self.__ovs_driver.is_interface_attached(
                ovs_bridge_name,
                veth_endpoint_a)
            present_in_container = self.__veth_driver.is_interface_present(
                veth_endpoint_b,
                netns=container_pid)
